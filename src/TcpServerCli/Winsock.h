...
namespace SXN::Net
{
	...
	private:
		...
		(
			...
			LPFN_DISCONNECTEX pDisconnectEx,
			LPFN_GETACCEPTEXSOCKADDRS pGetAcceptExSockaddrs,
			RIO_EXTENSION_FUNCTION_TABLE& rioFunctionsTable
		)
		{
	...
	public:
		...
		static Winsock* Initialize(SOCKET socket)
		{
			...
			{
				// get pointer
				auto getResult = GetExtensionFunctionAddress(socket, WSAID_ACCEPTEX, &pAcceptEx);
			...
			{
				// get pointer
				auto getResult = GetExtensionFunctionAddress(socket, WSAID_DISCONNECTEX, &pDisconnectEx);
